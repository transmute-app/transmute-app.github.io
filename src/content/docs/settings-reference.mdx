...
You can save a default `low`, `medium`, or `high` quality level for any output format that exposes quality options. When you later convert to that format, Transmute applies your saved default automatically unless you explicitly choose a different quality for that conversion.

See [Quality Reference](/docs/quality-reference) for the exact encoder settings behind each quality tier.
