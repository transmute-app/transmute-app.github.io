...
| `APP_URL` | `""` | Public URL of the app. Used for constructing URLs in the OIDC API response and for the displayed URL in the API docs. If it includes a path (e.g. `https://host/transmute`), Transmute is served under that [sub-path](/docs/reverse-proxy). |
